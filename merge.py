...
def get_ip_info():
    """Get local and public IP info to confirm environment."""
    try:
        hostname = socket.gethostname()
        local_ip = socket.gethostbyname(hostname)
        # Get public IP from an external service
        public_ip = requests.get("https://api.ipify.org", timeout=5).text
        return f"Local IP: {local_ip}, Public IP: {public_ip} (Filtering depends on this environment)"
    except Exception as e:
        return f"Could not determine IP info: {e}"
...
def merge_playlists():
    ...
    print("="*60)
    print(get_ip_info())
    print("⚠️ Reminder: Run this script on your Bangladeshi ISP (IP 103.73.185.252) for correct filtering.")
    print("="*60)
